...
## Generate vectors and drawables

The build plugin accepts regular fonts and fixed instances of variable fonts,
then generates their codepoints as vectors and drawables:
...
adds it as a runtime resource. `font("rounded-variable.ttf")` searches
`src/main/res/font` and `src/commonMain/composeResources/font`; omit it when
those locations contain only one font, or use `font.set(...)` for any other
path. Generated Kotlin has typed completion such as `AppIcons.Rounded.Home`;
native and Compose XML use one resource per unique codepoint. Every codepoint
is generated by default, while `include(...)` opts into a smaller subset.
Android projects default to the package `<android namespace>.generated`.
